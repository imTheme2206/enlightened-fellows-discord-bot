...
export function mergeSumMaps(maps: Array<Record<string, number>>): Record<string, number> {
...
export function hasNeededSkill(pieceSkills: Record<string, number>, neededSkills: Record<string, number>): boolean {
  return Object.keys(pieceSkills).some((sk) => sk in neededSkills)
}

export function isInSets(piece: ArmorPiece, setSkills: Record<string, number>): boolean {
  return piece.setSkills.some((sk) => sk in setSkills)
}

export function isInGroups(piece: ArmorPiece, groupSkills: Record<string, number>): boolean {
...
export function emptyGearPiece(type: SlotType, rank: string): Record<string, ArmorPiece> {
  return {
    None: {
      name: 'None',
...
export function getBestDecos(skills: Record<string, number>, allDecos: DecorationItem[]): Record<string, DecorationItem> {
...
export function getSkillPotential(
  ...
  const modPoints = points + leftoverSlots.filter((s) => otherDecoSlotSizes.includes(s)).length
...
): [Record<string, Record<string, ArmorPiece>>, Record<string, Record<string, ArmorPiece>>] {
...
export const skillRelevanceScore = (piece: ArmorPiece, skills: Record<string, number>, decos: Record<string, DecorationItem>): number => {
  let score = 0
