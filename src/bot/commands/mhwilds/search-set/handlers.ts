...
async function runSearch(interaction: MessageComponentInteraction, state: SearchState): Promise<void> {
...
export async function handleComponent(interaction: MessageComponentInteraction): Promise<void> {
  ...
  if (interaction.customId === 'search-set:prev' || interaction.customId === 'search-set:next') return
