...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) => option.setName('monster').setDescription('Select monster').setRequired(true).setAutocomplete(true))
...
export async function handleAutocomplete(interaction: AutocompleteInteraction): Promise<void> {
  ...
  const filteredMonsters = MONSTER_LIST.filter((monster) => monster.toLowerCase().startsWith(focusedValue.toLowerCase()))

  await interaction.respond(filteredMonsters.map((monster) => ({ name: monster, value: monster })))
}
